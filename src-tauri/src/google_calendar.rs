...
pub async fn connect_google_calendar(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        let refresh_token = token.refresh_token.ok_or_else(|| {
            "Google에서 갱신 토큰을 보내지 않았습니다. 연결을 해제한 뒤 다시 로그인해주세요.".to_string()
        })?;
        ...
        Ok(GoogleOAuthResult { email: user_info.email })
    })
    .await
    .map_err(|error| error.to_string())?
}
...
fn fetch_events(
    ...
    loop {
        let mut url = reqwest::Url::parse(EVENTS_URL).map_err(|error| error.to_string())?;
        {
            ...
            if let Some(token) = sync_token.as_ref() {
                ...
                if let Some(value) = time_min.as_ref() { query.append_pair("timeMin", value); }
                if let Some(value) = time_max.as_ref() { query.append_pair("timeMax", value); }
            }
            if let Some(token) = page_token.as_ref() { query.append_pair("pageToken", token); }
        }

        let response = client.get(url).bearer_auth(access_token).send()
            .map_err(|error| format!("Google Calendar에 연결하지 못했습니다. ({error})"))?;
        if response.status().as_u16() == 410 {
            return Ok(GoogleCalendarSyncResult { events: vec![], next_sync_token: None, reset_required: true });
        }
        let page = response.error_for_status()
        ...
        events.extend(page.items.into_iter().map(|event| GoogleCalendarEvent {
            id: event.id,
            title: if event.summary.trim().is_empty() { "제목 없는 일정".into() } else { event.summary },
            status: event.status,
            start_date_time: event.start.as_ref().and_then(|value| value.date_time.clone()),
    ...
    Ok(GoogleCalendarSyncResult { events, next_sync_token, reset_required: false })
}
...
fn send_token_request(client: &Client, form: &[(&str, String)]) -> Result<TokenResponse, String> {
    ...
    if !response.status().is_success() {
        ...
        return Err(format!("Google 인증에 실패했습니다. ({status}: {})", compact_error(&detail)));
    }
    response.json::<TokenResponse>().map_err(|error| format!("Google 인증 응답을 읽지 못했습니다. ({error})"))
}

fn wait_for_oauth_callback(listener: TcpListener, redirect_uri: &str, expected_state: &str) -> Result<String, String> {
    ...
    while Instant::now() < deadline {
        match listener.accept() {
            Ok((mut stream, _)) => {
                ...
                let path = request.split_whitespace().nth(1).ok_or("잘못된 OAuth 콜백입니다.")?;
                ...
                let state = params.get("state").map(|value| value.as_ref()).unwrap_or("");
                ...
                if let Some(error) = error { return Err(format!("Google 로그인이 취소되었거나 거부되었습니다. ({error})")); }
                if !valid_state { return Err("Google 로그인 응답의 보안 검증에 실패했습니다.".into()); }
...
fn validate_client_id(client_id: &str) -> Result<(), String> {
    if client_id.trim().is_empty() { return Err("Google OAuth Client ID를 입력해주세요.".into()); }
...
fn http_client() -> Result<Client, String> {
    Client::builder().timeout(Duration::from_secs(30)).build().map_err(|error| error.to_string())
}
...
fn sha256(input: &[u8]) -> [u8; 32] {
    ...
    for block in data.chunks(64) {
        let mut w = [0_u32; 64];
        for (index, word) in block.chunks(4).take(16).enumerate() { w[index] = u32::from_be_bytes([word[0],word[1],word[2],word[3]]); }
        for index in 16..64 {
        ...
        for (target, value) in h.iter_mut().zip([a,b,c,d,e,f,g,hh]) { *target = target.wrapping_add(value); }
    }
    let mut output = [0_u8; 32];
    for (chunk, value) in output.chunks_mut(4).zip(h) { chunk.copy_from_slice(&value.to_be_bytes()); }
    output
}
