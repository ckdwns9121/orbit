...
| 현재 결정 | 현재 배포 대상에서 사용되지 않으므로 `not_used`로 분류 |
...
- `cargo tree --target all -i glib`은 Tauri의 Linux GTK/WebKitGTK 경로에서 `glib 0.18.5`가 유입됨을 보여준다.
- `cargo tree --target aarch64-apple-darwin -i glib`과 `cargo tree --target x86_64-apple-darwin -i glib`은 의존 경로를 반환하지 않는다.
- 릴리스 워크플로는 `universal-apple-darwin`과 `app,dmg`만 빌드한다.
- 현재 Tauri 2 개발 브랜치도 Linux에서 `gtk 0.18`을 사용한다. Tauri maintainer는 임시 Cargo patch 또는 GTK 4로 이동하는 Tauri 3을 대안으로 안내했다: <https://github.com/tauri-apps/tauri/issues/12919#issuecomment-2706407744>

### Boundaries

- 이 판단은 macOS 릴리스에만 적용된다. Linux 개발·CI·배포에서는 취약 경로가 활성화될 수 있다.
...
재검토 시 Linux 대상 테스트, `cargo tree --target all -i glib`, 업스트림 Tauri 의존성 및 GitHub advisory 상태를 다시 확인한다.
