...
import { createWorkItemLink } from "./work-item-link-repository";
...
export async function discoverTaskContext(
  ...
  onProgress({ percent: 36, label: "담당 Jira 티켓을 가져오고 있어요" });
  try {
  ...
  onProgress({ percent: 62, label: "수집한 컨텍스트를 정리하고 있어요" });
  const sessionCandidates = buildSessionCandidates(taskTitle, sessions);

  const jiraCandidates = sortContextCandidates(taskTitle, issues
  ...
  const pool = [...sessionCandidates, ...jiraCandidates];
  if (pool.length === 0) {
    onProgress({ percent: 100, label: "확인할 수 있는 컨텍스트가 없어요" });
    return { candidates: [], usedAi: false, notices };
  }

  onProgress({ percent: 76, label: "AI가 Task와의 관련도를 분석하고 있어요" });
  ...
  const candidates = pool
    .map((candidate) => {
      ...
      const localScore = contextMatchScore(taskTitle, candidate);
      return {
        ...candidate,
        score: ai?.score ?? localScore,
        reason: ai?.reason || (localScore > 0 ? "Task 제목과 공통된 맥락이 있어요" : candidate.reason),
      };
    })
...
export async function autoConnectTaskAiSessions(
  ...
  const pool = buildSessionCandidates(taskTitle, sessions);
